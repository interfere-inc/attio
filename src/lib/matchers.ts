...
import { AttioDefaultError } from "../models/errors/attio-default-error.js";
...
export function match<T, E>(
...
): MatchFunc<T, E | AttioDefaultError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<T, E | AttioDefaultError | ResponseValidationError>,
      raw: unknown,
    ]
  > {
    let raw: unknown;
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new AttioDefaultError("Unexpected Status or Content-Type", {
          response,
          request,
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new AttioDefaultError("API error occurred", {
          request,
          response,
          body,
        }),
