/*
 * Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
 */

import * as z from "zod/v4-mini";
import { remap as remap$ } from "../../lib/primitives.js";
import * as types from "../../types/primitives.js";
import * as operations from "../operations/index.js";
import { AttioError } from "./attio-error.js";

/**
 * Not Found
 */
export type PatchV2TasksTaskIdNotFoundErrorData = {
  statusCode: number;
  type: operations.PatchV2TasksTaskIdNotFoundType;
  code: operations.PatchV2TasksTaskIdNotFoundCode;
  message: string;
};

/**
 * Not Found
 */
export class PatchV2TasksTaskIdNotFoundError extends AttioError {
  type: operations.PatchV2TasksTaskIdNotFoundType;
  code: operations.PatchV2TasksTaskIdNotFoundCode;

  /** The original data that was passed to this error instance. */
  data$: PatchV2TasksTaskIdNotFoundErrorData;

  constructor(
    err: PatchV2TasksTaskIdNotFoundErrorData,
    httpMeta: { response: Response; request: Request; body: string },
  ) {
    const message = err.message || `API error occurred: ${JSON.stringify(err)}`;
    super(message, httpMeta);
    this.data$ = err;
    this.type = err.type;
    this.code = err.code;

    this.name = "PatchV2TasksTaskIdNotFoundError";
  }
}

/**
 * Bad Request
 */
export type PatchV2TasksTaskIdValidationTypeErrorData = {
  statusCode: number;
  type: operations.PatchV2TasksTaskIdBadRequestType;
  code: operations.PatchV2TasksTaskIdCodeValidationType;
  message: string;
};

/**
 * Bad Request
 */
export class PatchV2TasksTaskIdValidationTypeError extends AttioError {
  type: operations.PatchV2TasksTaskIdBadRequestType;
  code: operations.PatchV2TasksTaskIdCodeValidationType;

  /** The original data that was passed to this error instance. */
  data$: PatchV2TasksTaskIdValidationTypeErrorData;

  constructor(
    err: PatchV2TasksTaskIdValidationTypeErrorData,
    httpMeta: { response: Response; request: Request; body: string },
  ) {
    const message = err.message || `API error occurred: ${JSON.stringify(err)}`;
    super(message, httpMeta);
    this.data$ = err;
    this.type = err.type;
    this.code = err.code;

    this.name = "PatchV2TasksTaskIdValidationTypeError";
  }
}

/** @internal */
export const PatchV2TasksTaskIdNotFoundError$inboundSchema: z.ZodMiniType<
  PatchV2TasksTaskIdNotFoundError,
  unknown
> = z.pipe(
  z.object({
    status_code: types.number(),
    type: operations.PatchV2TasksTaskIdNotFoundType$inboundSchema,
    code: operations.PatchV2TasksTaskIdNotFoundCode$inboundSchema,
    message: types.string(),
    request$: z.custom<Request>(x => x instanceof Request),
    response$: z.custom<Response>(x => x instanceof Response),
    body$: z.string(),
  }),
  z.transform((v) => {
    const remapped = remap$(v, {
      "status_code": "statusCode",
    });

    return new PatchV2TasksTaskIdNotFoundError(remapped, {
      request: v.request$,
      response: v.response$,
      body: v.body$,
    });
  }),
);

/** @internal */
export const PatchV2TasksTaskIdValidationTypeError$inboundSchema: z.ZodMiniType<
  PatchV2TasksTaskIdValidationTypeError,
  unknown
> = z.pipe(
  z.object({
    status_code: types.number(),
    type: operations.PatchV2TasksTaskIdBadRequestType$inboundSchema,
    code: operations.PatchV2TasksTaskIdCodeValidationType$inboundSchema,
    message: types.string(),
    request$: z.custom<Request>(x => x instanceof Request),
    response$: z.custom<Response>(x => x instanceof Response),
    body$: z.string(),
  }),
  z.transform((v) => {
    const remapped = remap$(v, {
      "status_code": "statusCode",
    });

    return new PatchV2TasksTaskIdValidationTypeError(remapped, {
      request: v.request$,
      response: v.response$,
      body: v.body$,
    });
  }),
);
