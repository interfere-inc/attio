 ...
 * Forbidden
 */
export type UpdateAttributeStatusAuthErrorData = {
  type: "auth_error";
  statusCode: 403;
  code: string;
  message: string;
 ...
 * Forbidden
 */
export class UpdateAttributeStatusAuthError extends AttioBaseError {
  type: "auth_error";
  code: string;

  /** The original data that was passed to this error instance. */
  data$: UpdateAttributeStatusAuthErrorData;

  constructor(
    err: UpdateAttributeStatusAuthErrorData,
    httpMeta: { response: Response; request: Request; body: string },
  ) {
    const message = err.message || `API error occurred: ${JSON.stringify(err)}`;
    super(message, httpMeta);
    ...
    this.name = "UpdateAttributeStatusAuthError";
  }
}
...
/** @internal */
export const UpdateAttributeStatusAuthError$inboundSchema: z.ZodMiniType<
...
> = z.pipe(
  ...
  z.transform((v) => {
    ...
    return new UpdateAttributeStatusAuthError(remapped, {
      request: v.request$,
      response: v.response$,
      body: v.body$,
    });
