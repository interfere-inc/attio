 ...
 */
export function scimUsersGet(
  client: AttioCore,
  request: operations.GetScimUserRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.GetScimUserResponse,
    | AttioBaseError
...
> {
  const parsed = safeParse(
    request,
    (value) => z.parse(operations.GetScimUserRequest$outboundSchema, value),
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetScimUserResponse$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
