 ...
 */
export function scimUsersDelete(
  client: AttioCore,
  request: operations.DeleteScimUserRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.DeleteScimUserResponse,
    | AttioBaseError
...
> {
  const parsed = safeParse(
    request,
    (value) => z.parse(operations.DeleteScimUserRequest$outboundSchema, value),
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(204, operations.DeleteScimUserResponse$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
