 ...
 */
export function scimGroupsGet(
  client: AttioCore,
  request: operations.GetScimGroupRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.GetScimGroupResponse,
    | AttioBaseError
...
> {
  const parsed = safeParse(
    request,
    (value) => z.parse(operations.GetScimGroupRequest$outboundSchema, value),
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetScimGroupResponse$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
